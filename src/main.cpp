...
const string strMessageMagic = "LightBurden Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // LightBurden: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // LightBurden: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
/* Início Adaptação para pagamentos Foundation */
    int64_t devCoin = 0 * COIN; // A cada bloco minerado em POW, 0.015 vai para a Equipe LightBurden.
...
// LightBurden: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes LightBurden fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // LightBurden: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // LightBurden: clean up wallet after disconnecting coinstake
    BOOST_FOREACH(CTransaction& tx, vtx)
...
/* Fim Adaptação para pagamentos Foundation */
    ...
    if (IsProofOfStake())
    {
        // LightBurden: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // LightBurden: track money supply and mint amount info
...
// LightBurden: total coin age spent in transaction, in the unit of coin-days.
...
// LightBurden: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // LightBurden: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // LightBurden: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // LightBurden: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // LightBurden: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig, int height) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // Lightburden: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // LightBurden: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // LightBurden: check proof-of-stake
    ...
    // LightBurden: ask for pending sync-checkpoint if any
...
printf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), pblock->hashPrevBlock.ToString().substr(0,20).c_str());
  if (pfrom) {
      ...
      // Ask this guy to fill in what we're missing
          ...
          // LightBurden: getblocks may not obtain the ancestor block rejected
    ...
    // LightBurden: if responsible for sync-checkpoint send it
...
// lightburden: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LightBurden", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // LightBurden: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // LightBurden: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // LightBurden: tell downloading node about the latest block if it's
