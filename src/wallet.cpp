...
// LightBurden: total coins staked (non-spendable until maturity)
...
bool CWallet::StakeForCharity()
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() == 0  && pcoin->GetDepthInMainChain() == nCoinbaseMaturity+10)
        {
            ...
            {
                SendMoneyToDestination(strStakeForCharityAddress.Get(), nNet, wtx, false); // LBDN removed 2 args  , "", true);
...
// Lightburden: get current stake weight
...
// LightBurden: check 'spent' consistency between wallet and txindex
// LightBurden: fix wallet spent state according to txindex
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, int& nOrphansFound, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s lbdn %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s lbdn %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), hash.ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
// LightBurden: disable transaction (only for coinstake)
