...
public:
    ...
    {
        // LightBurden: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // LightBurden: get transaction coin age
...
public:
    ...
    // LightBurden: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // LightBurden: two types of block: proof-of-work or proof-of-stake
    ...
    // LightBurden: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // LightBurden: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // LightBurden: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // LightBurden: block index flags
    enum
    {
