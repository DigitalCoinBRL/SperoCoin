...
void Shutdown(void* parg)
{
    ...
    RenameThread("LightBurden-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("LightBurden version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  LightBurdend [options]                     " + "\n" +
                  "  LightBurdend [options] <command> [params]  " + _("Send command to -server or LightBurdend") + "\n" +
                  "  LightBurdend [options] help                " + _("List commands") + "\n" +
                  "  LightBurdend [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "lightburden:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("LightBurden"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("LightBurden"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: LightBurden.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: LightBurdend.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 65820 or testnet: 66820)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 65821 or testnet: 66821)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  LightBurden is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("LightBurden version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "LightBurden server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("LightBurden"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // LightBurden: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // LightBurden: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of LightBurden") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart LightBurden to complete") << "\n";
